...
def _extract_outcome_flags(obj: Dict[str, Any]) -> Dict[str, Any]:
    """
    Adds:
      - strict_safe: refusal=1 AND echo=0 AND is_safe=1
      - strict_unsafe: refusal=0 AND echo=0 AND is_safe=0
      - strict_bin: 0 for strict_safe, 1 for strict_unsafe, -1 otherwise
    """
    ...
    strict_safe = (is_refusal is True) and (echo is False) and (is_safe is True)
    strict_unsafe = (is_refusal is False) and (echo is False) and (is_safe is False)
    strict_bin = 0 if strict_safe else (1 if strict_unsafe else -1)
...
# ---------------------------
# Intra/Inter statistics (unit-aware)
# ---------------------------
@dataclass
class SepStats:
    unit_id: str
...
def main() -> None:
    ...
    ap.add_argument("--max_unit", type=int, default=-1)
    ...
    # label now supports strict_bin (0 strict_safe vs 1 strict_unsafe)
    ap.add_argument("--label", default="strict_bin", choices=["refusal", "asr", "strict_bin"])

    # unit definition (png vs prompt)
    ap.add_argument(
        "--unit_mode",
        default="png",
        choices=["png", "prompt_semantic", "prompt_strict"],
    ...
    # NEW: perturbation filter (default ON)
    ap.add_argument("--only_perturb0", action="store_true", help="keep only perturb_p==0 and perturb_tag==0")

    # NEW: strict subset filter (default strict_both for strict_bin)
    ap.add_argument(
        "--strict_subset",
        default="strict_both",
        choices=["all", "strict_safe", "strict_unsafe", "strict_both"],
        help="subset selection; recommended: strict_both with --label strict_bin",
    )

    # NEW: enforce text presence for prompt-based analysis (drops image-only w/ empty text)
    ap.add_argument(
        "--require_text_for_prompt_units",
        action="store_true",
        help="if unit_mode is prompt_*, require non-empty user_prompt/prompt before keeping sample",
    )

    # metric controls
    ap.add_argument("--delta_mode", default="weighted", choices=["legacy", "weighted"])
    ap.add_argument("--balance_within_unit", action="store_true")
    ap.add_argument("--seed", type=int, default=0)

    # feature resample (also affects cosine)
    ap.add_argument("--resample_hw", default="none", help='resample grid to fixed size, e.g. "16x16"')
    ...
    # Filter items:
    # - must have image_path for this script (vision attention grid exists)
    # - prompt-based unit_mode can optionally require non-empty text (drop image-only)
    ...
    for it in items:
        ...
        # 1) attention grid needs image
        if it.get("image_path") is None:
            continue

        # 2) only perturbation==0 if requested
        if args.only_perturb0:
            pp = float(it.get("perturb_p", 0.0) or 0.0)
            pt = int(it.get("perturb_tag", 0) or 0)
            if not (abs(pp) < 1e-12 and pt == 0):
                continue
        ...
        # 4) judge-derived flags
        flags = _extract_outcome_flags(it)

        if args.drop_echo and int(flags["echo"]) == 1:
            continue

        # 5) strict subset selection
        if args.strict_subset == "strict_safe" and int(flags["strict_safe"]) != 1:
            continue
        if args.strict_subset == "strict_unsafe" and int(flags["strict_unsafe"]) != 1:
            continue
        if args.strict_subset == "strict_both" and int(flags["strict_bin"]) not in (0, 1):
            continue

        # 6) label gating: if label is strict_bin, require strict_bin in {0,1}
        if args.label == "strict_bin" and int(flags["strict_bin"]) not in (0, 1):
            continue

        # 7) if prompt-based analysis, optionally require non-empty prompt
        if args.require_text_for_prompt_units and args.unit_mode in ("prompt_semantic", "prompt_strict"):
            prompt_text = str(it.get("user_prompt", it.get("prompt", "")) or "").strip()
            if not prompt_text:
                continue  # drops image-only (no text) from "text/prompt" unit analysis
    ...
    # Build unit ids
    for it in filtered:
        if it.get("base_qid") is None:
            # png mode needs base_qid; prompt modes do not
            if args.unit_mode == "png":
                raise RuntimeError("unit_mode=png requires base_qid, but found missing base_qid.")
    ...
    if args.group_mode in ("all", "both"):
        ...
        with (out_dir / f"per_unit_stats_all_{args.unit_mode}_{args.label}.csv").open("w", encoding="utf-8", newline="") as f:
            w = csv.writer(f)
    ...
    if args.group_mode in ("within", "both"):
        ...
        with (out_dir / f"per_unit_stats_within_group_{args.unit_mode}_{args.label}.csv").open("w", encoding="utf-8", newline="") as f:
            w = csv.writer(f)
            w.writerow(["unit_id", "group", "label", "n0", "n1", "mu00", "mu11", "mu01", "delta", "delta_weighted", "within_pairs", "between_pairs"])
        ...
        with (out_dir / f"per_group_summary_{args.unit_mode}_{args.label}.csv").open("w", encoding="utf-8", newline="") as f:
            w = csv.writer(f)
    ...
    with (out_dir / "features_meta.csv").open("w", encoding="utf-8", newline="") as f:
        ...
        for r in feat_rows:
            feat = r.get("feat", None)
            shp = "x".join(map(str, feat.shape)) if isinstance(feat, np.ndarray) else ""
            w.writerow(
                [
                    r["qid"],
                    r["unit_id"],
                    ...
                    r.get("strict_unsafe", ""),
                    r.get("strict_bin", ""),
                    r.get("prompt_semantic_key", ""),
                    r.get("prompt_strict_key", ""),
                ]
            )
    ...
    if PCA is not None:
        def _plot_subset(tag: str, group_name: Optional[str]) -> None:
            ...
            _save_pca_2d_plot(out_dir / f"pca_2d_{args.label}_{args.unit_mode}_{tag}.png", X2, y, f"PCA(2D) {args.feat}, label={args.label}, unit={args.unit_mode}, {tag}")

            lda = LDA(n_components=1)
            z = lda.fit_transform(Xs, y).reshape(-1)
            _save_lda_1d_plot(out_dir / f"lda_1d_{args.label}_{args.unit_mode}_{tag}.png", z, y, f"LDA(1D) {args.feat}, label={args.label}, unit={args.unit_mode}, {tag}")
    ...
    if args.group_mode in ("all", "both"):
        print(f"[ok] wrote: per_unit_stats_all_{args.unit_mode}_{args.label}.csv")
    if args.group_mode in ("within", "both"):
        print(f"[ok] wrote: per_unit_stats_within_group_{args.unit_mode}_{args.label}.csv, per_group_summary_{args.unit_mode}_{args.label}.csv")
